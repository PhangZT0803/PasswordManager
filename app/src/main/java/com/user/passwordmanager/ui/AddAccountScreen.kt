...
fun AddAccountScreen(viewModel: AccountViewModel,navController: NavController){
    ...
    topBar = {
        ...
        Column(
            ...
            Button(
                onClick = {
                    if (webSiteName.isNotBlank() && userName.isNotBlank() && password.isNotBlank()) {
                        viewModel.addAccount(webSiteName, userName, password)
                        navController.popBackStack()
                    }
                },
                modifier = Modifier.fillMaxWidth(),
                enabled = webSiteName.isNotBlank() && userName.isNotBlank() && password.isNotBlank()
            ) {
                Text("Add Account")
            }
        }
